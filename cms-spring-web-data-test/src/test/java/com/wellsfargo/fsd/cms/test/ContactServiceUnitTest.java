...
public class ContactServiceUnitTest {
	...
	@Test
	public void whenSave_givenNoContact_shouldThrowException() throws ContactException {
		Contact contact = new Contact(201, "test1",LocalDate.now(),"2031456789","testgrp",false);
		Mockito.when(contactRepo.existsById(contact.getContactId())).thenReturn(true);
		//assertThrows(ContactException.class,()->{contactService.save(contact);});
		assertEquals(contactService.save(contact),contact);
	}
	
	@Test
	public void whenDelete_givenContactID_shouldDeleteSuccess() throws ContactException {
		Contact contact = new Contact(201, "test1",LocalDate.now(),"2031456789","testgrp",false);
		Mockito.when(contactRepo.existsById(contact.getContactId())).thenReturn(false);
		//assertEquals(contactService.deleteContact(contact.getContactId()),true);
		assertThrows(ContactException.class, ()->{contactService.deleteContact(contact.getContactId());});
	}
}
